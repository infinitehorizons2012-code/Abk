...
def process_subject_dir(subj_path, grade, day_str, day_num, subj_clean, legacy_key):
    book_data = {}
    ubd_data = {}
    interactive_data = {}
    whisper_segs = []
    ...
    # Use scandir to list files in subject dir in 1 single call!
    if os.path.exists(subj_path) and os.path.isdir(subj_path):
        try:
            with os.scandir(subj_path) as entries:
...
def main():
    ...
    if os.path.exists(g5_dir):
        with os.scandir(g5_dir) as day_entries:
            day_dirs = sorted([e for e in day_entries if e.is_dir()], key=lambda x: x.name)
            for day_entry in day_dirs:
                day_folder = day_entry.name
                day_path = day_entry.path
                ...
                try:
                    with os.scandir(day_path) as sub_entries:
                        subject_dirs = [s.name for s in sub_entries if s.is_dir()]
                except Exception:
                    subject_dirs = []

                if not subject_dirs:
                    subject_dirs = g5_subjects
                ...
                for subj in subject_dirs:
                    ...
                    all_lessons[legacy_key] = process_subject_dir(subj_path, "Grade 5", day_str, day_num, subj_clean, legacy_key)

    print("--- Processing Grade 3 ---", flush=True)
    if os.path.exists(g3_dir):
        with os.scandir(g3_dir) as day_entries:
            day_dirs = sorted([e for e in day_entries if e.is_dir()], key=lambda x: x.name)
            for day_entry in day_dirs:
                day_folder = day_entry.name
                day_path = day_entry.path
                ...
                for subj in subject_dirs:
                    ...
                    subj_path = os.path.join(day_path, subj)
                    all_lessons[key_slug] = process_subject_dir(subj_path, "Grade 3", day_str, day_num, subj_clean, key_slug)
