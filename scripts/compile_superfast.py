...
def is_subfolder_name(name):
    # Subject folders don't have extension dots
    return not ('.' in name or name.endswith('.gdoc'))
...
def main():
    ...
    if os.path.exists(g5_dir):
        day_folders = [d for d in os.listdir(g5_dir) if is_subfolder_name(d)]
        for day_folder in day_folders:
            ...
            sub_items = [s for s in os.listdir(day_path) if is_subfolder_name(s)]
            if not sub_items:
                ...
                for subj in sub_items:
                    ...
                    if subj_clean in ['test', 'desktop.ini']:
                        continue
                    key_slug = f"g5-d{day_num:03d}-{subj_clean.lower().replace(' ', '-')}"
                    legacy_key = "arithmetic-5" if (day_num == 1 and "arithmetic" in subj_clean.lower()) else key_slug
    ...
    print(f"Grade 5 done in {time.time() - t0:.2f}s. --- Processing Grade 3 ---", flush=True)
    if os.path.exists(g3_dir):
        day_folders = [d for d in os.listdir(g3_dir) if is_subfolder_name(d)]
        for day_folder in day_folders:
    ...
    print(f"Total compiled lessons: {len(all_lessons)} in {time.time() - t0:.2f}s", flush=True)
